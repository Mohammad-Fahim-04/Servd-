...
- Do not identify containers, utensils, or packaging.
- Be specific.
- Estimate realistic quantities.
- Confidence must be between 0.7 and 1.0.
- Maximum 20 items.
- Common pantry staples are acceptable.
`;
    ...
    throw new Error(
      error?.message || "Failed to scan image"
    );
  }
}
...
export async function saveToPantry(formData) {
  try {
    ...
    throw new Error(
      error?.message || "Failed to save items"
    );
  }
}
...
export async function getPantryItems() {
  try {
    ...
    throw new Error(
      error?.message || "Failed to load pantry"
    );
  }
}
