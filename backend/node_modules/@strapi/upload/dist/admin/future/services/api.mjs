...
import { openUploadProgress, setUploadFailed, retryCancelledFiles, setFileError, setFileComplete, setFileUploading, setFileMetadataGenerating, setFileMetadataResult, setFileProgress } from '../store/uploadProgress.mjs';
...
const registerUploadEntries = (uploadId, entries, generateAiMetadata, concurrency)=>{
...
/** Maps a server per-file outcome onto the row's terminal metadata status. */ const METADATA_STATUS_BY_RESULT = {
    success: 'generated',
    skipped: 'skipped',
    error: 'failed'
};
 ...
 */ const maybeGenerateMetadata = ({ file, index, uploadId, enabled, dispatch })=>{
 ...
 */ const runUploadPool = async ({ entries, indices, token, uploadId, abortController, dispatch, concurrency = 1, generateAiMetadata })=>{
 ...
 */ const processSSEStream = async ({ response, dispatch, uploadId, generateAiMetadata })=>{
...
const { useUploadFilesMutation, useUploadFileSilentlyMutation, useRetryCancelledFilesMutation, useUploadFromUrlsMutation, useGenerateAiMetadataMutation } = uploadApi;

export { abortUpload, maybeGenerateMetadata, uploadApi, useGenerateAiMetadataMutation, useRetryCancelledFilesMutation, useUploadFileSilentlyMutation, useUploadFilesMutation, useUploadFromUrlsMutation };
