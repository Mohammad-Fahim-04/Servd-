...
const AssetsDndProvider = ({ children })=>{
    ...
    const handleDragStart = React.useCallback((event)=>{
        ...
        const { items, fromSelection } = buildDragSet.buildDragSet(data, selection?.selectedKeys, currentFolderId);
        dragSessionRef.current = {
            items,
            fromSelection
        };
    ...
    const handleDragEnd = React.useCallback(async (event)=>{
        ...
        const successMessage = formatMoveSuccessMessage.formatMoveSuccessMessage({
            formatMessage,
            count: items.length,
            source: getFolderLabel.getFolderLabel(folderStructure, currentFolderId, rootLabel),
    ...
    const announcements = React.useMemo(()=>({
            ...
            onDragEnd: ({ active, over })=>{
                ...
                const { items } = buildDragSet.buildDragSet(activeData, selection?.selectedKeys, currentFolderId);
                if (!canDropItemOnFolder.canDropItemOnFolder({
                    items,
